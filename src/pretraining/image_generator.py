...
class MammoData():
    def __init__(self):
        self.df = self.restructure_data(pd.read_csv("data/EMBED/tables/merged_df.csv",
            low_memory=False))
...
def load_model(model_path, device='cuda', checkpoint=False): 
    if checkpoint:
        mammo_model = MultiViewModel.load_from_checkpoint(model_path, class_weights=np.empty([2], dtype=float))
...
def reconstruct_image(model, image_path, device='cuda'):
    source_view = imread(image_path).astype(np.float32) / 65535.0 
...
def main(hparams)
    ...
    model = load_model(hparams.model_path, checkpoint=True)
    while True:
        idx = int(input("Enter an index to reconstruct an image: "))
        mlo = int(input("Enter 1 for MLO->CC or 2 for CC->MLO: ")) == 1
        mlo_path, cc_path = mammo_data.get_random_sample_pair(idx)
        output_image = reconstruct_image(model, (mlo_path if mlo else cc_path), device='cuda')
